...
--/*
--- Manually Running sharewatch Stored Procedures 
--Per share
...
--Once Share passes onboarding checklist, setup share for monitoring. Review Onboarding checklist in sharewatch Whitepaper for details
-- Enable sharewatch monitor on shared Db
call sharewatch.util.setup_schemadrift_monitor('mutating_share_db');
call sharewatch.util.setup_schemadrift_monitor('clean_room_party1');

-- Run monitor to check status of shares
call sharewatch.util.run_schemadrift_monitor('mutating_share_db');

-- Run monitor for all shares being watched
call sharewatch.util.run_schemadrift_monitor();

-- Drop monitor on a share
call sharewatch.util.drop_schemadrift_monitor('mutating_share_db');
...
--call sharewatch.util.update_app_config('notificiation_email','{"email_address": "amit.gupta@snowflake.com"}');

--configure master task
--call sharewatch.util.configure_master_task ('0/2 * * * *  America/New_York'); -- Run Every 2 mins
...
call sharewatch.util.update_app_config('notification_email','{"email_address": "amit.gupta@snowflake.com"}');
